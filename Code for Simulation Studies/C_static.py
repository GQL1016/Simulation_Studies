...
# 设置全局随机种子
global_seed = 2025
# 使用全局随机种子初始化 NumPy 的全局随机数生成器, 使其在每次程序执行时产生相同的随机数序列
...
"""
                 ==================================================================================
                    ...
                    Module B (Static): 基于模型 (控制组和实验组) 生成观测数据,但不计算最优带宽等有关结果
...
print(f"状态: Static / 控制组: {Control_Idm} - 实验组: {Trial_Idm} / 观测向量维度: {d} / 隐变量个数: {r} / 观测向量个数: {n}")
print()  # 这将打印一个空行
...
print(f"Module B (Static) 总执行时间: {total_ModuleB_time:.4f} 秒")
print()  # 这将打印一个空行
...
"""
                                   ...
                                   Required Auxiliary Functions A (Static)

"""


# 计算 真实稀疏部分 tSparse 和 真实低秩部分 tLowrank
...
"""
            ====================================================================================
                ...
                Module C (Static): 基于控制组和实验组生成的整个观测向量序列 , 选取与之匹配的最优的参数组合
...
# 并行处理的任务
def process_comb(w,TrueRankIner,\
    ...
    # 打印当前参数组合, 便于跟踪
    print_info = (f"[Static w={w}] "
                  f"参数组合: [alphahat={alphahat}, betahat={betahat}, shatprob={shatprob}, rhat={rhat}]")
    ...
    for X_traingroup, X_testgroup, Y_traingroup, Y_testgroup in split_pairs:
        ...
        # 使用 A 训练, B 测试
        Result1 = OurTwoStageMethod(w,X_traingroup, Y_traingroup,\
                                    np.zeros((X_traingroup.shape[1] - 1, X_traingroup.shape[1] - 1)),\
                                    np.zeros((Y_traingroup.shape[1] - 1, Y_traingroup.shape[1] - 1)),\
                                    TrueRankIner,\
                                    alphahat, betahat, shatprob, rhat)

        # Idend 变量用来表示算法是否收敛. 具体来说, Idend 的值为 1 表示算法尚未收敛, 而值为 0 表示算法已经收敛
        if Result1.Idend == 0:
            Sigmahat_X_testgroup_cov = np.cov(X_testgroup[:, 1:], rowvar=False)
            # X_testgroup[:, 1:]是一个二维数组, 每一行表示一个样本, 每一列表示一个特征 ;
            # rowvar=False: 假定每一列是一个变量(特征), 每一行是一个观察值(样本).
            Sigmahat_Y_testgroup_cov = np.cov(Y_testgroup[:, 1:], rowvar=False)
            testloss1 = Loss(Sigmahat_X_testgroup_cov, Sigmahat_Y_testgroup_cov, Result1)
            loss_values.append(testloss1)

        # # 使用 B 训练, A 测试
        Result2 = OurTwoStageMethod(w,X_testgroup, Y_testgroup,\
                                    np.zeros((X_testgroup.shape[1] - 1, X_testgroup.shape[1] - 1)),\
                                    np.zeros((Y_testgroup.shape[1] - 1, Y_testgroup.shape[1] - 1)),\
                                    TrueRankIner,\
                                    alphahat, betahat, shatprob, rhat)

        # Idend 变量用来表示算法是否收敛. 具体来说, Idend 的值为 1 表示算法尚未收敛, 而值为 0 表示算法已经收敛
        if Result2.Idend == 0:
            Sigmahat_X_traingroup_cov = np.cov(X_traingroup[:, 1:], rowvar=False)
            Sigmahat_Y_traingroup_cov = np.cov(Y_traingroup[:, 1:], rowvar=False)
            testloss2 = Loss(Sigmahat_X_traingroup_cov, Sigmahat_Y_traingroup_cov, Result2)
...
# 主函数: 基于控制组和实验组生成的整个观测向量序列, 选取与之匹配的最优参数组合 [alphahat, betahat, shatprob, rhat]
def ChooseComb(w, TrueRankIner, \
               X_combined_vectors, Y_combined_vectors, global_seed,\
               random_split_times = 2, \
               Alphahat=[0.01, 0.03, 0.05, 0.5], Betahat=[1, 3], Shatprob=[1, 3, 5], Rhat=[0, 1, 2, 3, 4]):


    split_counter = global_seed  # 初始化 split_counter, 用于设置数据分割的随机种子
    ...
    for alphahat in Alphahat:
        for betahat in Betahat:
            for shatprob in Shatprob:
                for rhat in Rhat:
                    tasks.append(
                        delayed(process_comb)(w,TrueRankIner,\
                                              alphahat, betahat, shatprob, rhat, split_pairs))
...
"""
                 ================================================================================
                        ...
                        Module D (Static): 针对不同时刻点 w , 评估真实与静态估计的各项差异并汇总结果
...
def OurApproach(Control_Idm, Trial_Idm, d, r, n,\
                X_combined_vectors, Y_combined_vectors, global_seed):
    ...
    for w in w_values:
        ...
        # # 虽然 w 动态变化, 但其协方差估计矩阵保持静态
        # np.cov(np.copy(X_combined_vectors)[:, 1:], rowvar=False)
        # np.cov(np.copy(Y_combined_vectors)[:, 1:], rowvar=False)

        # 虽然 w 动态变化, 但其选择的 最优参数组合[alphahat, betahat, shatprob, rhat] 保持一致
        selected_params_w, total_split_count_w = ChooseComb(w,TrueRankIner_w,\
                            np.copy(X_combined_vectors), np.copy(Y_combined_vectors),\
                            global_seed)

        # selected_params_w 中参数的顺序是 [alphahat, betahat, shatprob, rhat]
        print_info = f"[Static w={w}] 最优参数组合: " \
        ...
        # 运行两阶段算法
        Result_w = OurTwoStageMethod(w, np.copy(X_combined_vectors), np.copy(Y_combined_vectors),\
...
def save_summary_to_file(SumRes, Control_Idm, Trial_Idm, d, r, n, filename="Summary_Static_Results.txt"):
    with open(filename, 'w') as file:  # 使用写入模式，确保每次调用都覆盖之前的文件内容
        ...
        file.write("\n=============== Static TrueR1_Recovery Count ===============\n")
        file.write(f"Static TrueR1_Recovery Count = {TrueR1_Recovery_count:<6}\n")

        '''
            Static TrueR_Recovery
        '''
        ...
        file.write("\n=============== Static TrueR_Recovery Count ===============\n")
        file.write(f"Static TrueR_Recovery Count = {TrueR_Recovery_count:<6}\n")
        ...
        file.write("\n=============== Static TPR Median & Standard Deviation ===============\n")
        file.write(f"Static TPR Median = {TPR_median:<8.6f}\n")
        file.write(f"Static TPR Standard Deviation = {TPR_std:<8.6f}\n")
        ...
        file.write("\n=============== Static FPR Median & Standard Deviation ===============\n")
        file.write(f"Static FPR Median = {FPR_median:<8.6f}\n")
        file.write(f"Static FPR Standard Deviation = {FPR_std:<8.6f}\n")

        '''
            Static SparseFrobError
        '''
        ...
        file.write("\n=============== Static SparseFrobError Median & Standard Deviation ===============\n")
        file.write(f"Static SparseFrobError Median = {SparseFrobError_median:<8.6f}\n")
        file.write(f"Static SparseFrobError Standard Deviation = {SparseFrobError_std:<8.6f}\n")

        '''
           Static LowrankFrobError
        '''
        ...
        file.write("\n=============== Static LowrankFrobError Median & Standard Deviation ===============\n")
        file.write(f"Static LowrankFrobError Median = {LowrankFrobError_median:<8.6f}\n")
        file.write(f"Static LowrankFrobError Standard Deviation = {LowrankFrobError_std:<8.6f}\n")

        '''
            Static TotalFrobError
        '''
        ...
        file.write("\n=============== Static TotalFrobError Median & Standard Deviation ===============\n")
        file.write(f"Static TotalFrobError Median = {TotalFrobError_median:<8.6f}\n")
        file.write(f"Static TotalFrobError Standard Deviation = {TotalFrobError_std:<8.6f}\n")

        '''
            Static SparseSpecError
        '''
        ...
        file.write("\n=============== Static SparseSpecError Median & Standard Deviation ===============\n")
        file.write(f"Static SparseSpecError Median = {SparseSpecError_median:<8.6f}\n")
        file.write(f"Static SparseSpecError Standard Deviation = {SparseSpecError_std:<8.6f}\n")

        '''
            Static LowrankSpecError
        '''
        ...
        file.write("\n=============== Static LowrankSpecError Median & Standard Deviation ===============\n")
        file.write(f"Static LowrankSpecError Median = {LowrankSpecError_median:<8.6f}\n")
        file.write(f"Static LowrankSpecError Standard Deviation = {LowrankSpecError_std:<8.6f}\n")

        '''
            Static TotalSpecError
        '''
        ...
        file.write("\n=============== Static TotalSpecError Median & Standard Deviation ===============\n")
        file.write(f"Static TotalSpecError Median = {TotalSpecError_median:<8.6f}\n")
        file.write(f"Static TotalSpecError Standard Deviation = {TotalSpecError_std:<8.6f}\n")
...
SumRes = OurApproach(Control_Idm, Trial_Idm,\
                      d, r, n,\
                      X_combined_vectors, Y_combined_vectors,\
                      global_seed)
