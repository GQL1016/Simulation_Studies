...
def OurInitial(X_group, Y_group, alphahat, betahat, shat, rhat, d):
    n1 = X_group.shape[0]
    n2 = Y_group.shape[0]

    Sigmahat_X_group_cov = np.cov(X_group[:, 1:], rowvar=False)
    Sigmahat_Y_group_cov = np.cov(Y_group[:, 1:], rowvar=False)

    TildeSigmahat_X_group_cov = (n1 - 1) / (n1 - d - 2) * Sigmahat_X_group_cov
    TildeSigmahat_Y_group_cov = (n2 - 1) / (n2 - d - 2) * Sigmahat_Y_group_cov

    Deltahat = np.linalg.inv(TildeSigmahat_X_group_cov) - np.linalg.inv(TildeSigmahat_Y_group_cov)

    JsDeltahat = HardTrun(Deltahat, int(shat))  # 硬阈值截断操作, 强制转换 shat 为整数
...
# maxIter: 最大迭代次数
# eps: 精度误差
def OurConverg(X_group, Y_group, S0, U0, L0, tSparse, tLowrank, \
    ...
    # 计算差异和rhat
    hatDiff = Sigmahat_Y_group_cov - Sigmahat_X_group_cov
    rhat = np.size(U0, 1)
    ...
    while k <= maxIter:  # maxIter: 最大迭代次数
        # 计算一些公共项
        Term1 = S0 + L0
        Term2 = np.matmul(np.matmul(Sigmahat_X_group_cov, Term1), Sigmahat_Y_group_cov)
        Term3 = np.matmul(np.matmul(Sigmahat_Y_group_cov, Term1), Sigmahat_X_group_cov)
        ...
        ObjLoss = LossSampCov(ObjLoss, Sigmahat_X_group_cov, Sigmahat_Y_group_cov, S1, U1, L1, r1hat)
        # 论文中 (7) 式的目标优化函数
...
def OurTwoStageMethod(w, X_group, Y_group, \
                      tSparse, tLowrank, \
                      TrueRankIner, \
    ...
    # 运行阶段-I算法
    S0, U0, L0, r1hat = OurInitial(X_group, Y_group, \
                                   alphahat, betahat, shat, rhat, d)
    ...
    ObjLoss = LossSampCov(ObjLoss, Sigmahat_X_group_cov, Sigmahat_Y_group_cov, S0, U0, L0, r1hat)
    ...
    Result = OurConverg(X_group, Y_group, S0, U0, L0, tSparse, tLowrank, \
                        r1hat, alphahat, betahat, shat, eta1, eta2, TrueError, ObjLoss, Result, maxIter, eps)
